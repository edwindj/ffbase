...
merge.ffdf <- function(x, y, by = intersect(names(x), names(y)), by.x = by, by.y = by, all=FALSE, all.x = all, all.y = all, sort=FALSE, 
  ...
  if(jointype == "inner"){
  	...
  	for(i in 1:length(yvars)){
  		x[[newyvars[i]]] <- ff::clone.ff(y[[yvars[i]]], initdata=NA, length = nrow(x), vmode = tocoerce$coerceto[[yvars[i]]])
  	} 
  	...
  	if(length(tocoerce.yvars) > 0){
  		...
  		for(measure in tocoerce.yvars){
 				y[[measure]] <- ff::clone.ff(y[[measure]], vmode = tocoerce$coerceto[[measure]])
  		}
  	} 		
